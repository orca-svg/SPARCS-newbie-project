...
import { updateMemberRoleTierSchema } from "../validation/schemas.js";
import { ClubService } from "../services/clubs.service.js";
export class ClubController {
    ...
    static async requestJoin(req, res) {
        try {
            if (!req.user) {
    ...
    static async listJoinRequests(req, res) {
        try {
            ...
            const requests = await ClubService.listJoinRequests(clubId, req.user.userId, req.user.role);
    ...
    static async approveMember(req, res) {
        try {
            ...
            const updated = await ClubService.approveMember(clubId, memberId, req.user.userId, req.user.role);
    ...
    static async rejectMember(req, res) {
        try {
            ...
            const result = await ClubService.rejectMember(clubId, memberId, req.user.userId, req.user.role);
            return res.json(result);
        }
        catch (e) {
            return res.status(400).json({
    ...
    static async create(req, res) {
        try {
            ...
            const data = parseBody(createClubSchema, req.body);
            const normalized = data.name.trim().toLowerCase();
            const allClubs = await ClubService.listAll();
            const exists = allClubs?.some((c) => typeof c.name === "string" &&
    ...
    static async listMembers(req, res) {
        try {
            ...
            const members = await ClubService.listMembers(clubId, req.user.userId);
    ...
    static async updateMemberRoleTier(req, res) {
        try {
            if (!req.user) {
                return res.status(401).json({ message: "로그인이 필요합니다." });
            }
            ...
            const updated = await ClubService.updateMemberRoleTier(req.user.userId, clubId, memberId, data);
            return res.json({ member: updated });
        }
        catch (e) {
