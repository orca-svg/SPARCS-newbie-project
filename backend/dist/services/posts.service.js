...
export class PostService {
    ...
    static async ensureClubMember(userId, clubId) {
    ...
    static async getClubMember(userId, clubId) {
    ...
    static async createPost(clubId, userId, data) {
        await this.ensureClubMember(userId, clubId);
    ...
    // 댓글 목록 (멤버만)
    static async listComments(postId, userId) {
        const post = await prisma.post.findUnique({
            where: { id: postId },
        });
        if (!post) {
    ...
    // 게시글 수정 (작성자 본인만)
    static async updatePost(postId, userId, clubId, data) {
        await this.ensureClubMember(userId, clubId);
        ...
        if (typeof data.isNotice === "boolean" &&
        ...
        const updated = await prisma.post.update({
            ...
            data: {
                ...
                isNotice: typeof data.isNotice === "boolean"
                    ? data.isNotice
                    : existing.isNotice,
            },
        });
